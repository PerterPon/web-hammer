...
var istanbulVar = phantom.args[ 4 ];
...
function injectJs( page ) {
  ...
  page.injectJs( dirName + '/res/mocha.js' );
  page.injectJs( dirName + '/res/extensions.js' );
  page.injectJs( dirName + '/res/expect.js' );
  page.injectJs( dirName + '/res/init.js' );
  page.injectJs( dirName + '/node_modules/node-cube/runtime/cube.js' );
}
...
function initPage () {
  ...
  page.open( 'http://' + hostname + ':' + masterPort + '/blank', phantomReady );
...
function initServer () {
  ...
  if ( service ) {
    ...
    console.log( 'phantom server start with error!' );
    exit();
  }
}
...
function evaluate( data ) {
  ...
  page.evaluate( function( script, istanbulVar, coverage ) {
...
function runScriptDone() {
  page.evaluate( function( luantaiParams ) {
    window.__runScriptDone( luantaiParams, window._test_ );
  }, luantaiParams );
}
